...
if(isset($_POST['item_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE t_news_item SET s_headline = :headline , s_content = :content , s_slug = :slug , s_secret = :secret WHERE pk_i_id = :id"); 
    ...
    $stmt = $conn->prepare("INSERT INTO t_news_item (fk_i_user_id, s_headline, s_slug, s_content, s_create_time, s_secret) VALUES (:fkuid, :headline, :slug, :content, :time, :secret)");
    $stmt->bindParam(':fkuid', $uid);
    $stmt->bindParam(':headline', $headline);
    $stmt->bindParam(':slug', $slug);
    $stmt->bindParam(':content', $content);
