...
<tr>  
  <th class="tg-6k2t" >Categories</th>
    <th class="tg-6k2t" >
    <?php
    if ($edit == true) {?>
      <script type="text/javascript">  $(".textarea").hide(); </script>
      <select name="categories" id="categories" style="width: 400;">
    ...
    while ($allCategories = $query->fetch(PDO::FETCH_ASSOC)) { ?>
      ...
      { ?>
      <textarea class ="textarea" style="width: 400;" value="<?php echo $allCategories['pk_i_id']; ?>  "<?php if(!$edit) echo "disabled" ;?>><?php echo $categories['categories_name']; ?></textarea>
    <?php  } 
    else { ?>
      <script type="text/javascript">  $(".textarea").hide(); </script>
      <select name="categories" id="categories" style="width: 400;">
